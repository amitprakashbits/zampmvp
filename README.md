...
- **Triages** the queue by revenue-at-risk and works the biggest dollars first
...
- **Drafts** the actual outreach copy, and reports a **confidence** on every call
...
- **Proves incrementality** - holds out an untouched control and reports true
  lift, so it never takes credit for users who'd convert anyway
- Runs inside operator **guardrails**; everything is logged and **exportable**
- Takes **delegation** via a built-in assistant and a **⌘K command palette**

It demonstrates what an "AI employee" has to do - monitor, act, escalate, learn,
and prove its ROI - with every decision legible and auditable.

The interface is a deliberate, restrained ops-SaaS dashboard (Linear / Ramp /
Mercury lineage): tight neutral palette, one calm accent, monospace for numbers,
guided onboarding, skeleton loading, and dashboard-grade dropdowns - not a
generic "AI app" look.
